  ...
  <div class="container">
    ...
    <c:choose>
      <c:when test="${empty sessionScope.user}">
        <p class="error-message">パスキー登録はログイン済みユーザーのみ利用できます。先に通常ログインしてください。</p>
        <p><a href="<%= request.getContextPath() %>/login.jsp">ログインページへ移動</a></p>
      </c:when>
      <c:otherwise>
        <p>このページはログイン済みユーザー向けのパスキー登録ページです。ボタンを押すとパスキー作成フローが開始します。</p>
      ...
      btn.addEventListener('click', async () => {
        try {
          ...
          if (!startRes.ok) {
            ...
            throw new Error('作成開始に失敗しました。' + (txt ? ' サーバ応答: ' + txt : ''));
          }
          ...
          if (!credential) throw new Error('Credential の作成に失敗しました。');
          ...
          if (!finishRes.ok) {
            ...
            throw new Error('登録完了でエラーが発生しました。' + (txt ? ' サーバ応答: ' + txt : ''));
          }
